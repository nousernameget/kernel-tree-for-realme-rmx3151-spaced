...
static int policy_lookup(struct dm_cache_policy *p, dm_oblock_t oblock, dm_cblock_t *cblock,
				int data_dir, bool fast_copy, bool *background_queued)
{
	return p->lookup(p, oblock, cblock, data_dir, fast_copy, background_queued);
}

static int policy_lookup_with_work(struct dm_cache_policy *p,
...
static int policy_get_background_work(struct dm_cache_policy *p,
					     bool idle, struct policy_work **result)
{
	return p->get_background_work(p, idle, result);
}

static void policy_complete_background_work(struct dm_cache_policy *p,
						   struct policy_work *work,
						   bool success)
{
...
static void policy_set_dirty(struct dm_cache_policy *p, dm_cblock_t cblock)
{
	p->set_dirty(p, cblock);
}

static void policy_clear_dirty(struct dm_cache_policy *p, dm_cblock_t cblock)
{
	p->clear_dirty(p, cblock);
}

static int policy_load_mapping(struct dm_cache_policy *p,
				      dm_oblock_t oblock, dm_cblock_t cblock,
...
static int policy_invalidate_mapping(struct dm_cache_policy *p,
					    dm_cblock_t cblock)
{
	return p->invalidate_mapping(p, cblock);
}

static uint32_t policy_get_hint(struct dm_cache_policy *p,
				       dm_cblock_t cblock)
{
	return p->get_hint ? p->get_hint(p, cblock) : 0;
}

static dm_cblock_t policy_residency(struct dm_cache_policy *p)
{
	return p->residency(p);
}

static void policy_tick(struct dm_cache_policy *p, bool can_block)
{
	if (p->tick)
		return p->tick(p, can_block);
}

static int policy_emit_config_values(struct dm_cache_policy *p, char *result,
...
static int policy_set_config_value(struct dm_cache_policy *p,
					  const char *key, const char *value)
{
	return p->set_config_value ? p->set_config_value(p, key, value) : -EINVAL;
}

static void policy_allow_migrations(struct dm_cache_policy *p, bool allow)
{
