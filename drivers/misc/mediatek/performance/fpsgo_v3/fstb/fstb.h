...
static int switch_fps_range(int nr_level,
	struct fps_level *level) { return 0; }
static int switch_process_fps_range(char *proc_name,
	int nr_level, struct fps_level *level) { return 0; }
static int switch_thread_fps_range(pid_t pid,
	int nr_level, struct fps_level *level) { return 0; }
...
static int fpsgo_fbt2fstb_update_cpu_frame_info(
	int pid,
	unsigned long long bufID,
	int tgid,
	int frame_type,
...
static void fpsgo_fbt2fstb_query_fps(int pid,
		int *target_fps, int *target_cpu_time, int *fps_margin,
		int tgid, unsigned long long mid, int *quantile_cpu_time,
		int *quantile_gpu_time) { }
...
/* EARA */
static void eara2fstb_get_tfps(int max_cnt, int *pid,
		unsigned long long *buf_id, int *tfps) { }
static void eara2fstb_tfps_mdiff(int pid, unsigned long long buf_id,
		int diff) { }
