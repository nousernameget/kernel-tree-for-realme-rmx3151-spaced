...
static struct imu_pgd_t *imu_supersection_start(struct imu_pgd_t *pgd)
{
	return (struct imu_pgd_t *)(round_down((unsigned long)pgd, (16 * 4)));
}
static struct imu_pte_t *imu_largepage_start(struct imu_pte_t *pte)
{
	return (struct imu_pte_t *)(round_down((unsigned long)pte, (16 * 4)));
}

static unsigned long long m4u_calc_next_mva(unsigned long long addr,
